...
/**
 * ================================================================
 * FACIÊNCIA ERP - MÓDULO DE HISTÓRICOS ESCOLARES
 * ================================================================
 * 
 * Sistema de gerenciamento de históricos acadêmicos
 * Permite gerar, visualizar e gerenciar históricos escolares
 * em formato PDF com dados acadêmicos completos
 * 
 * @version 2.0.0
 ...
 * 
 * Funcionalidades:
 * - Geração de históricos escolares em PDF
 * - Exibição de notas e disciplinas cursadas
...
$titulo_pagina = 'Históricos Escolares';       // Título padrão da página
...
/**
 * Busca dados completos do aluno para geração de históricos
 * Inclui informações do curso, turma, polo, matrícula e notas
 * 
 * @param object $db Instância do banco de dados
...
function executarConsultaAll($db, $sql, $params = [], $default = []) {
    try {
        ...
        error_log('executarConsultaAll - Result count: ' . ($result ? count($result) : 0));        return $result ?: $default;
    } catch (Exception $e) {
        // Registra o erro no log do sistema
...
switch ($action) {
    ...
    // ============================================================
      case 'baixar_em_lote':
        // Exibe a interface para configurar download em lote de históricos
    ...
    case 'processar_download_lote':
        ...
        if (empty($tipo_documento_id)) {
            ...
            header('Location: historico.php?action=baixar_em_lote');
            exit;
        }
    ...
    case 'gerar_documentos_multiplos':
        ...
        header('Location: historico.php?action=mostrar_progresso');
        exit;
    ...
    case 'gerar_documento_solicitacao':
        ...
        if (strpos($tipo_documento, 'declaração') !== false || strpos($tipo_documento, 'declaracao') !== false) {
            ...
            header("Location: declaracoes.php?action=gerar_declaracao&aluno_id={$aluno_id}&solicitacao_id={$solicitacao_id}");
            exit;
            ...
            header('Location: historicos.php');
            exit;
        }
               ...
               <div class="container mx-auto">
                   ...
                   switch ($view) {
                       case 'selecionar_aluno':
                           ...
                           break;
                       case 'progresso':
                           include 'views/historicos/progresso.php';
                           break;
                       case 'listar':
                           include 'views/historicos/listar.php';
                           break;
                       case 'configuracoes':
                           include 'views/historicos/configuracoes.php';
                           break;
                       case 'diagnostico':
                           include 'views/historicos/diagnostico.php';
                           break;
                       case 'opcoes_declaracao':
                           include 'views/historicos/opcoes_declaracao.php';
                           break;
                       case 'baixar_em_lote':
                           error_log("Tentando incluir o arquivo: views/historicos/baixar_em_lote.php");
                           if (file_exists('views/historicos/baixar_em_lote.php')) {
                               error_log("Arquivo views/historicos/baixar_em_lote.php existe");
                               include 'views/historicos/baixar_em_lote.php';
                               error_log("Arquivo views/historicos/baixar_em_lote.php incluído com sucesso");
                           } else {
                               error_log("ERRO: Arquivo views/historicos/baixar_em_lote.php não existe");
                           }
                           break;
                       case 'inicio':
                       default:
