...
<div class="mb-4 flex justify-between items-center">
    <a href="historico.php" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded">
        <i class="fas fa-arrow-left mr-2"></i> Voltar
    </a>
    <div class="flex space-x-2">
        <a href="historico.php?action=selecionar_aluno" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <div class="p-6 border-b border-gray-200">
        <h3 class="text-xl font-semibold text-gray-800">Selecionar Alunos para Históricos Acadêmicos</h3>
        <p class="text-gray-600 mt-1">Selecione os alunos para gerar históricos acadêmicos em lote.</p>
    </div>
    <div class="p-6">
        <!-- Formulário de busca -->
        <form action="historico.php" method="get" class="mb-6">
        ...
        <?php if (isset($alunos) && count($alunos) > 0): ?>
            <div class="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 mb-4">
                <h4 class="font-bold text-blue-800 mb-2">Instruções para geração de históricos:</h4>
                <ul class="list-disc pl-5 space-y-1">
                    <li>Selecione um ou mais alunos na lista abaixo</li>
                    <li>Clique em "Gerar Históricos" para processar os documentos</li>
                    <li>Os históricos serão gerados em lotes para evitar sobrecarga do servidor</li>
                    <li>O processo pode levar alguns minutos para turmas grandes</li>
                    <li>Você também pode usar o botão flutuante no canto inferior direito da tela</li>
                </ul>
                <p class="mt-2 font-medium">Aguarde até que todos os documentos sejam gerados. Não feche a página durante o processamento.</p>
            </div>

            <form action="historico.php" method="post" id="form-documentos-multiplos">
                ...
                <input type="hidden" name="tipo_documento" value="historico">
                ...
                <div class="mb-4">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <!-- Opções para históricos -->
                        <div class="flex items-center space-x-4">
                            <div class="flex items-center">
                                <input type="checkbox" id="incluir_notas" name="incluir_notas" value="1" checked class="mr-2 h-5 w-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500">
                                <label for="incluir_notas" class="text-sm font-medium text-gray-700">Incluir notas nos históricos</label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end space-x-2">
                        <button type="submit" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded text-sm">
                            <i class="fas fa-graduation-cap mr-1"></i> Gerar Históricos Acadêmicos
                        </button>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($alunos as $aluno): ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-4 border-b">
                                        <div class="flex space-x-2">
                                            <a href="historico.php?action=gerar_historico&aluno_id=<?php echo $aluno['id']; ?>" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm" title="Gerar Histórico Acadêmico">
                                                <i class="fas fa-graduation-cap mr-1"></i> Histórico
                                            </a>
                                        </div>
                                    </td>
                                </tr>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    document.getElementById('form-documentos-multiplos').addEventListener('submit', function(e) {
                        ...
                        // Sempre será histórico nesta página
                        const tipoDocumentoTexto = 'históricos';
                ...
                // Botão para gerar históricos
                const gerarHistoricosBtn = document.createElement('button');
                gerarHistoricosBtn.innerHTML = '<i class="fas fa-graduation-cap"></i>';
                gerarHistoricosBtn.className = 'bg-green-500 hover:bg-green-600 text-white rounded-full w-12 h-12 flex items-center justify-center shadow-lg';
                gerarHistoricosBtn.title = 'Gerar Históricos';

                // Adiciona os botões ao container
                actionButtons.appendChild(scrollToTopBtn);
                actionButtons.appendChild(gerarHistoricosBtn);
                ...
                // Gera históricos quando o botão é clicado
                gerarHistoricosBtn.addEventListener('click', function() {
                    const alunosSelecionados = document.querySelectorAll('.checkbox-aluno:checked');
