    ...
    if ($action === 'salvar') {
        try {
            ...
            if ($contaId) {
                ...
                $db->update('contas_pagar', $dados, 'id = ?', [$contaId]);
                $_SESSION['success'] = 'Conta a pagar atualizada com sucesso!';
            } else {
                $db->insert('contas_pagar', $dados);
                $_SESSION['success'] = 'Conta a pagar cadastrada com sucesso!';
            }
...
if ($action === 'editar' && $contaId) {
    try {
        $conta = $db->fetchOne("SELECT * FROM contas_pagar WHERE id = ?", [$contaId]);
        if (!$conta) {
...
try {
    $categorias = $db->fetchAll("
        ...
        WHERE tipo = 'despesa' AND status = 'ativo' 
        ORDER BY nome
    ");
} catch (Exception $e) {
    $categorias = [];
...
if ($action === 'listar') {
    ...
    if ($busca) {
        $where .= " AND (descricao LIKE ? OR fornecedor_nome LIKE ?)";
        $params[] = "%$busca%";
        $params[] = "%$busca%";
    }
...
<body class="bg-gray-100">
                ...
                <div class="max-w-7xl mx-auto">
                    ...
                    <div class="mb-8">
                        <div class="flex justify-between items-center mb-6">
                            <div>
                                ...
                                <p class="text-gray-600 mt-2">Gerencie as contas a pagar da instituição</p>
                            </div>
                            <a href="contas_pagar.php?action=nova" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700">
                        ...
                        <div class="bg-white rounded-lg shadow p-4 mb-6">
                            <form method="GET" class="flex flex-wrap gap-4 items-end">
                                ...
                                <div>
                                    ...
                                    <select name="filtro" class="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a href="contas_pagar.php?action=editar&id=<?php echo $conta['id']; ?>" 
                                               class="text-red-600 hover:text-red-900 mr-3">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <?php if ($conta['status'] === 'pendente'): ?>
                                            <button onclick="abrirModalPagamento(<?php echo $conta['id']; ?>, '<?php echo htmlspecialchars($conta['descricao']); ?>', <?php echo $conta['valor']; ?>)" 
                    ...
                    <div class="mb-8">
                        <div class="flex items-center mb-6">
                            <a href="contas_pagar.php" class="text-red-600 hover:text-red-800 mr-4">
                                <i class="fas fa-arrow-left"></i>
                            </a>
                            <div>
                                <h1 class="text-3xl font-bold text-gray-900">
                                    <?php echo $action === 'nova' ? 'Nova Conta a Pagar' : 'Editar Conta a Pagar'; ?>
                                </h1>
                                <p class="text-gray-600 mt-2">Preencha os dados da conta a pagar</p>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="fornecedor_nome" 
                                           value="<?php echo htmlspecialchars($conta['fornecedor_nome'] ?? ''); ?>"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
                                </div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="observacoes" rows="3" 
                                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"><?php echo htmlspecialchars($conta['observacoes'] ?? ''); ?></textarea>
                                </div>
                            </div>
                            
                            <div class="mt-8 flex justify-end space-x-3">
                                <a href="contas_pagar.php" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400">
                                    Cancelar
                                </a>
                                <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700">
    ...
    <script>
    function abrirModalPagamento(id, descricao, valor) {
        const modal = Financeiro.Modal.show('Registrar Pagamento', `
            <form id="form-pagamento" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="forma_pagamento" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500">
        ...
        `, {
            ...
            onConfirm: `document.getElementById('form-pagamento').action = 'contas_pagar.php?action=pagar&id=${id}'; document.getElementById('form-pagamento').submit();`
        });
    }
    </script>
</body>
