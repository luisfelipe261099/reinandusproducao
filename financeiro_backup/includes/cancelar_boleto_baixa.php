<?php
/**
 * Função para cancelar boletos via API de baixa do Itaú
 */

/**
 * Cancela um boleto bancário via API de baixa do Itaú
 *
 * @param int $boleto_id ID do boleto a ser cancelado
 * @param object $db Objeto de conexão com o banco de dados
 * @return array Resultado da operação
 */
function cancelarBoletoBaixa($boleto_id, $db) {
    try {
        // Log para depuração
        error_log("Iniciando cancelamento via API de baixa do boleto ID: $boleto_id");
        ...
        // Formata o nosso número conforme o padrão do Itaú (sem carteira e dígito verificador)
        $nosso_numero = $boleto['nosso_numero'];

        // Log do nosso número original
        error_log("Nosso número original: " . $nosso_numero);

        // Baixa URL com o nosso número formatado
        $baixa_url     = "https://api.itau.com.br/cobranca/v2/boletos/{$nosso_numero}/baixas";
        ...
        error_log("Nosso número usado na baixa: " . $boleto['nosso_numero']);
        ...
        if ($httpcode >= 200 && $httpcode < 300 || $is_echo_response) {
            ...
            if ($is_echo_response) {
                ...
                error_log("Boleto baixado com sucesso na API do Itaú: ID $boleto_id");
            }
            ...
            try {
                $db->insert('boletos_historico', [
                    'boleto_id' => $boleto_id,
                    'acao' => 'cancelamento_baixa',
                    'data' => date('Y-m-d H:i:s'),
                    'usuario_id' => isset($_SESSION['usuario_id']) ? $_SESSION['usuario_id'] : null,
                    'detalhes' => 'Cancelado via API de baixa do Itaú' . ($is_echo_response ? ' (resposta: eco do payload)' : '')
                ]);
            } catch (Exception $e) {
            ...
            return [
                'status' => 'sucesso',
                'mensagem' => 'Boleto cancelado com sucesso via API de baixa.' . ($is_echo_response ? ' A API retornou o payload como resposta, o que é considerado sucesso.' : '')
            ];
        } else {
            // Erro na baixa na API
            error_log("Erro ao baixar boleto na API do Itaú: ID $boleto_id, HTTP $httpcode, Resposta: $response");
            ...
            return [
                'status' => 'erro',
                'mensagem' => 'Erro ao cancelar boleto via API de baixa: ' . $response
            ];
        }
    } catch (Exception $e) {
        error_log('Erro ao cancelar boleto via API de baixa: ' . $e->getMessage());

        return [
            'status' => 'erro',
            'mensagem' => 'Erro ao cancelar boleto via API de baixa: ' . $e->getMessage()
        ];
    }
}
?>
