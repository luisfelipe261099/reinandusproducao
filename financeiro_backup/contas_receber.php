...
switch ($action) {
    case 'nova':
        ...
        if (!Auth::hasPermission('financeiro', 'criar')) {
            setMensagem('erro', 'Você não tem permissão para criar contas a receber.');
    ...
    case 'editar':
        ...
        if (!Auth::hasPermission('financeiro', 'editar')) {
            setMensagem('erro', 'Você não tem permissão para editar contas a receber.');
            redirect('contas_receber.php');
            exit;
        }
        ...
        if (!isset($_GET['id']) || empty($_GET['id'])) {
            setMensagem('erro', 'ID da conta a receber não informado.');
            redirect('contas_receber.php');
            exit;
        }
        ...
        if (!$conta) {
            setMensagem('erro', 'Conta a receber não encontrada.');
            redirect('contas_receber.php');
            exit;
        }
        ...
        // Define o título da página
        $titulo_pagina = 'Editar Conta a Receber';
        $view = 'form';
        break;
    ...
    case 'salvar':
        ...
        if (!Auth::hasPermission('financeiro', 'criar') && !Auth::hasPermission('financeiro', 'editar')) {
            setMensagem('erro', 'Você não tem permissão para criar/editar contas a receber.');
            redirect('contas_receber.php');
            exit;
        }
        ...
        if (!empty($data_recebimento) && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $data_recebimento)) {
            $erros[] = 'A data de recebimento deve estar no formato correto.';
        }
        ...
        if (!empty($erros)) {
            ...
            if ($id) {
                redirect("contas_receber.php?action=editar&id=$id");
            } else {
                redirect('contas_receber.php?action=nova');
            }
            exit;
        }
        ...
        try {
            ...
            // Se a conta foi recebida, cria uma transação
            if ($status === 'recebido' && $data_recebimento) {
                // Verifica se já existe uma transação associada
                $transacao_id = null;
                if ($id) {
                    $sql = "SELECT transacao_id FROM contas_receber WHERE id = ?";
                ...
                $dados_transacao = [
                    ...
                    'valor' => $valor,
                    'data_transacao' => $data_recebimento,
                    'categoria_id' => $categoria_id,
                ...
                // Se a conta não foi recebida, remove a transação associada, se houver
                if ($id) {
                    $sql = "SELECT transacao_id FROM contas_receber WHERE id = ?";
                    $result = $db->fetchOne($sql, [$id]);
                    $transacao_id = $result ? $result['transacao_id'] : null;
            ...
            // Atualiza ou insere a conta a receber
            if ($id) {
                $db->update('contas_receber', $dados, 'id = ?', [$id]);
                $mensagem = 'Conta a receber atualizada com sucesso.';
            } else {
                $dados['created_at'] = date('Y-m-d H:i:s');
                $id = $db->insert('contas_receber', $dados);
            ...
            setMensagem('erro', 'Erro ao salvar a conta a receber: ' . $e->getMessage());
    ...
    case 'excluir':
        ...
        if (!Auth::hasPermission('financeiro', 'excluir')) {
            setMensagem('erro', 'Você não tem permissão para excluir contas a receber.');
            redirect('contas_receber.php');
            exit;
        }
        ...
        try {
            ...
            $db->delete('contas_receber', 'id = ?', [$id]);

            // Confirma a transação
            $db->commit();

            // Define a mensagem de sucesso
            setMensagem('sucesso', 'Conta a receber excluída com sucesso.');
            ...
            // Define a mensagem de erro
            setMensagem('erro', 'Erro ao excluir a conta a receber: ' . $e->getMessage());
        }
    ...
    case 'receber':
        // Verifica permissão para editar
        if (!Auth::hasPermission('financeiro', 'editar')) {
            setMensagem('erro', 'Você não tem permissão para receber contas.');
            redirect('contas_receber.php');
            exit;
        }
        ...
        // Se a conta já foi recebida, redireciona
        if ($conta['status'] === 'recebido') {
            setMensagem('info', 'Esta conta já foi recebida.');
            redirect('contas_receber.php');
            exit;
        }
    ...
    case 'visualizar':
        ...
        // Busca a conta a receber com informações relacionadas
        $id = (int)$_GET['id'];
        $sql = "SELECT cr.*, c.nome as categoria_nome
                FROM contas_receber cr
                LEFT JOIN categorias_financeiras c ON cr.categoria_id = c.id
                WHERE cr.id = ?";
        $conta = $db->fetchOne($sql, [$id]);
        ...
        // Define o título da página
        $titulo_pagina = 'Visualizar Conta a Receber';
        $view = 'visualizar';
        break;
    ...
    default:
        ...
        if (isset($_GET['data_recebimento_inicio']) && !empty($_GET['data_recebimento_inicio'])) {
            $where[] = "cr.data_recebimento >= ?";
            $params[] = $_GET['data_recebimento_inicio'];
            $filtros['data_recebimento_inicio'] = $_GET['data_recebimento_inicio'];
        }

        if (isset($_GET['data_recebimento_fim']) && !empty($_GET['data_recebimento_fim'])) {
            $where[] = "cr.data_recebimento <= ?";
            $params[] = $_GET['data_recebimento_fim'];
            $filtros['data_recebimento_fim'] = $_GET['data_recebimento_fim'];
        }

        // Filtro por termo de busca
        if (isset($_GET['termo']) && !empty($_GET['termo'])) {
            $where[] = "(cr.descricao LIKE ? OR cr.cliente_nome LIKE ? OR cr.observacoes LIKE ?)";
            $params[] = '%' . $_GET['termo'] . '%';
            $params[] = '%' . $_GET['termo'] . '%';
            $params[] = '%' . $_GET['termo'] . '%';
        ...
        $sql = "SELECT COUNT(*) as total FROM contas_receber cr $whereClause";
        ...
        // Consulta para buscar as contas a receber
        $sql = "SELECT cr.*, c.nome as categoria_nome
                FROM contas_receber cr
                LEFT JOIN categorias_financeiras c ON cr.categoria_id = c.id
                $whereClause
...
<head>
    ...
    <style>
        ...
        /* Estilos específicos para contas a receber */
        .status-badge {
            display: inline-flex;
            align-items: center;
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
                <div class="container mx-auto">
                    ...
                    $view_file = __DIR__ . "/views/contas_receber/$view.php";
                    if (file_exists($view_file)) {
                        ...
                        echo '<p class="mt-4"><a href="contas_receber.php" class="text-blue-600 hover:text-blue-800">Voltar para a listagem</a></p>';
                        echo '</div>';
                    }
                    ?>
                </div>
