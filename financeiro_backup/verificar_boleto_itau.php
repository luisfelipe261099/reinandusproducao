<?php
/**
 * Página para verificar o status de um boleto no Itaú usando o formato exato
 */

// Inclui os arquivos necessários
require_once __DIR__ . '/../includes/init.php';
require_once __DIR__ . '/includes/consultar_boleto_itau_exato.php';
...
if ($action === 'verificar') {
    $resultado = consultarBoletoItauExato($boleto['nosso_numero'], $db);
...
$titulo_pagina = 'Verificar Boleto no Itaú';
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6">
                <div class="container mx-auto">
                    ...
                    <div class="bg-white shadow-md rounded-lg p-6">
                        <h1 class="text-2xl font-bold mb-6 text-purple-800">Verificar Boleto no Itaú</h1>

                        <div class="mb-6">
                            <div class="mb-6 p-4 bg-blue-50 border-l-4 border-blue-400 text-blue-700">
                                <p class="font-bold">Informação:</p>
                                <p>Esta página verifica o status do boleto no Itaú usando o formato exato do nosso número.</p>
                                <p class="mt-2">O sistema consultará o boleto usando o formato 109/XXXXXXXX-2, que é o formato que aparece no sistema do Itaú.</p>
                            </div>
                            ...
                            <div class="api-info-box">
                                ...
                                <div class="api-info-value">API de Consulta com Formato Exato</div>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="p-4 bg-green-50 border-l-4 border-green-400 text-green-700 mb-4">
                                    ...
                                    <p>O boleto foi encontrado no sistema do Itaú usando o formato: <?php echo $resultado['nosso_numero_formatado']; ?></p>
                                </div>
                                
                                <div class="bg-gray-100 p-4 rounded-lg mb-4">
                                    <?php 
                                    // Extrai o status do boleto
                                    $status_itau = 'desconhecido';
                                    $situacao_itau = '';
                                    
                                    if (isset($resultado['dados']['situacao'])) {
                                        $situacao_itau = $resultado['dados']['situacao'];
                                        
                                        // Mapeia a situação para um status
                                        switch (strtolower($situacao_itau)) {
                                            case 'aberto':
                                            case 'pendente':
                                                $status_itau = 'pendente';
                                                break;
                                            case 'pago':
                                            case 'liquidado':
                                                $status_itau = 'pago';
                                    ...
                                    <p><strong>Status no Itaú:</strong> 
                                        <span class="status-badge <?php 
                                            echo $status_itau === 'pendente' ? 'status-pendente' : 
                                                ($status_itau === 'pago' ? 'status-pago' : 
                                                    ($status_itau === 'cancelado' ? 'status-cancelado' : 
                                                        ($status_itau === 'vencido' ? 'status-vencido' : 'status-desconhecido'))); 
                                        ?>">
                                            <?php echo ucfirst($status_itau); ?>
                                        </span>
                                    </p>
                                    <p><strong>Situação no Itaú:</strong> <?php echo $situacao_itau; ?></p>
                                </div>
                                
                                <div class="result-box">
                                    <div class="result-title">Detalhes da Resposta:</div>
                                    <div class="result-value"><?php echo json_encode($resultado['dados'], JSON_PRETTY_PRINT); ?></div>
                                </div>
                                
                                <?php if ($status_itau !== $boleto['status']): ?>
                                <div class="p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700 mb-4">
                                    <p class="font-bold">Atenção:</p>
                                    <p>O status do boleto no Itaú (<?php echo ucfirst($status_itau); ?>) é diferente do status no sistema (<?php echo ucfirst($boleto['status']); ?>).</p>
                                    <p class="mt-2">Deseja atualizar o status no sistema para refletir o status no Itaú?</p>
                                    <div class="mt-4">
                                        <a href="atualizar_status_boleto.php?id=<?php echo $id; ?>&status=<?php echo $status_itau; ?>" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded">
                                ...
                                <div class="p-4 bg-red-50 border-l-4 border-red-400 text-red-700 mb-4">
                                    ...
                                    <?php if (isset($resultado['nosso_numero_formatado'])): ?>
                                    <p class="mt-2"><strong>Formato usado:</strong> <?php echo $resultado['nosso_numero_formatado']; ?></p>
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="flex flex-wrap gap-4">
                                <a href="verificar_boleto_itau.php?action=verificar&id=<?php echo $id; ?>" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
